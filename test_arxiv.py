"""Test script for arXiv search provider"""
import sys
import logging
from ai_scholar.providers.arxiv_search_provider import ArxivSearchProvider
...
def test_arxiv_search():
    ...
    print("=" * 60)
    print("Testing arXiv Search Provider")
    print("=" * 60)
    
    # Initialize provider
    print("\n1. Initializing arXiv provider...")
    provider = ArxivSearchProvider()
    print(f"   Provider name: {provider.get_provider_name()}")
    
    # Check availability
    print("\n2. Checking if arXiv is available...")
    ...
    print("\n4. Testing search with year filter (2023-2024)...")
    try:
        results_filtered = provider.search(query, limit, min_year=2023, max_year=2024)
